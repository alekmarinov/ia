...
ia_contours_p ia_contours_find(ia_image_p, ia_uint32_t);
...
static void          ia_contour_draw       (ia_contour_p, ia_image_p, ia_uint32_t);
...
static ia_bool_t     ia_contour_find_any   (ia_image_p, ia_image_p, ia_int32_t, ia_int32_t, ia_uint32_t, ia_int32_t*, ia_int32_t*);
static ia_bool_t     ia_contour_find_next  (ia_image_p, ia_image_p, ia_int32_t, ia_int32_t, ia_uint32_t, ia_int32_t*, ia_int32_t*, ia_int32_t *);
...
static ia_bool_t ia_contour_find_any(ia_image_p img, ia_image_p history, ia_int32_t start_x, ia_int32_t start_y, ia_uint32_t bkcolor, ia_int32_t* x, ia_int32_t* y)
{
	for (*y=start_y; (*y)<img->height; (*y)++)
		if ((*y)==start_y)
		{
			for (*x=start_x; (*x)<img->width;  (*x)++)
				if (!history->get_pixel(history, *x, *y) && (bkcolor != img->get_pixel(img, *x, *y)))
				{
						return 1;
				}
		}
...
/*
 * draw a contour with specified color
 */
static void ia_contour_draw(ia_contour_p self, ia_image_p img, ia_uint32_t color)
{
	ia_int32_t i;
	for (i=0; i<self->npoints; i++)
	{
		img->set_pixel(img, self->points[i]->x, self->points[i]->y, color);
	}
}
...
static ia_bool_t ia_contour_find_next(ia_image_p img, ia_image_p history, ia_int32_t start_j, ia_int32_t start_i, ia_uint32_t bkcolor, ia_int32_t* j, ia_int32_t* i, ia_int32_t *d)
{
	...
	do
	{
		...
		if (next_j>=0 && next_j<img->width && next_i>=0 && next_i<img->height)
		{
			...
			if (bkcolor != img->get_pixel(img, next_j, next_i))
			{
				*d=(nd+4)&7;
				*j=next_j;
				*i=next_i;
...
ia_contours_p ia_contours_find(ia_image_p img, ia_uint32_t bkcolor)
{
	...
	while (ia_contour_find_any(img, history, 0, 0, bkcolor, &x, &y))
	{
		/* container of pixels */
		ia_contour_p contour=ia_contour_new();
		...
		do
		{
			...
			prev_x=x; prev_y=y;
			find_result=ia_contour_find_next(img, history, start_x, start_y, bkcolor, &x, &y, &d);
			if (prev_y!=y)
			{
				if (prev_y>y)
				{
			...
			{
				prev_x=x; prev_y=y;
				ia_contour_find_next(img, history, start_x, start_y, bkcolor, &x, &y, &d);
				if (first_d != d)
				{
					find_result=1;
				} else
